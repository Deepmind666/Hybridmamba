...
if [[ -n "${SYSTEM_CUDA_VERSION}" && -n "${TORCH_CUDA_VERSION}" && "${SYSTEM_CUDA_VERSION}" != "${TORCH_CUDA_VERSION}" ]]; then
  SYSTEM_CUDA_MINOR="$(echo "${SYSTEM_CUDA_VERSION}" | awk -F. '{print $1 "." $2}')"
  TORCH_CUDA_MINOR="$(echo "${TORCH_CUDA_VERSION}" | awk -F. '{print $1 "." $2}')"
  if [[ "${SYSTEM_CUDA_MINOR}" != "${TORCH_CUDA_MINOR}" ]]; then
