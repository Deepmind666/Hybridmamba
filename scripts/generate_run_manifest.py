...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a run manifest from a detection config.")
...
def main() -> None:
    ...
    if not dataset_name:
        ann_file = cfg.get("data", {}).get("train", {}).get("ann_file", "")
        dataset_name = Path(ann_file).parts[-3] if ann_file else ""
